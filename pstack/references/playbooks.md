...
3. Plan the fix. If it crosses a function boundary, run Architect with high reasoning. Delegate a clear, bounded implementation to a Codex worker with `gpt-5.6-sol`, medium reasoning, and fast tier when supported. Use high when the bug is ambiguous or high risk.
