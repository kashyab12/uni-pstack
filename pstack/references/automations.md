...
- Execution: `codex exec` with `gpt-5.6-sol`, medium reasoning for the automation coordinator, and supported fast/priority tier unless overridden. Delegated routine workers and explorers use low. `gpt-5.6-sol` is capped at medium; the runner clamps high/xhigh down.
