...
Before spawning investigators, list the available external tools and MCP servers from the current host. In Codex, prefer the available tool list, MCP resource/tool discovery, and installed app/connectors exposed in the session. In Claude Code, use the currently available MCP/tool list. Only inspect host-specific configuration directories when the current environment explicitly exposes them.
...
Delegate each investigator with pstack's host-native rules. In Codex, use native Codex subagents. In Claude Code, use Codex CLI workers with `--role explorer`. Use the configured why-investigators model default: `gpt-5.6-sol` with medium reasoning and the supported fast/priority tier. Give investigators tool/MCP access when their evidence category needs it. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
Spawn one synthesizer using pstack's host-native delegation rules. In Claude Code, use `--role synthesizer`. Use the configured why-synthesizer model default: `gpt-5.6-sol` with high reasoning and the supported fast/priority tier. The synthesizer's quality check spot-verifies citations, which can require MCP access.
