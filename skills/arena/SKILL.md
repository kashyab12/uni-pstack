...
3. Pick the runners. Default runners are your configured arena list (defaults `gpt-5.5 high reasoning`, `gpt-5.5 high reasoning`, `gpt-5.5 high reasoning`). Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
