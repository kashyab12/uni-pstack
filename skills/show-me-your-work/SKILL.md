...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript from the current host's active thread history or a workspace-scoped transcript path named by the system/app context. Don't glob across global product history directories such as `~/.cursor/projects/*/`, unrelated Codex threads, or unrelated Claude projects; that reads unrelated private chats. Walk the log against what actually happened:
