...
- Pass model defaults explicitly when the host supports it: `gpt-5.6-sol` medium reasoning for routine code and exploration, high for judgment, synthesis, and high-risk work.
...
If a phase creates or edits a skill, the phase instructs the implementer to use the host skill-authoring workflow. In Codex, that means **skill-creator**.
...
**Runtime.** Exercise the feature on the matching surface via the relevant host control tool:

- Browser / Electron / Web UIs: browser automation, app automation, or the host UI-control skill.
- CLIs and TUIs: terminal interaction or the host CLI-control skill.
- Native mobile: whatever simulator-driving skill your team has.
- No control tool for the touched surface: flag it in the plan.
...
- the host's diff cleanup/review tools over each diff before commit. the **unslop** skill over any prose surface.
- the **show-me-your-work** skill to keep a decision trail when the plan is large enough to need an auditable record.
- host CI and PR review monitoring after opening the PR.
