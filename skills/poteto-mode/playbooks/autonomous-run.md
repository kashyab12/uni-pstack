...
2. Pick the host wake mechanism. An event to watch (CI, a merge, a ref advancing) gets a watcher, monitor, automation, or subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
